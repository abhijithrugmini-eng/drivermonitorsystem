...
- Loops a small canned set of lat/lon waypoints along a "highway" with an oscillating speed profile, calling the Telematics Agent's `run()` directly on a timer (e.g. every 1–2s) — no HTTP hop, no second process, nothing extra for a demo operator to start.
...
- **Do not grow this toward the Fleet Simulator's feature set** (multiple trucks, real route/geofence logic, a config file) — see `dms-agentic-architecture`'s "Telematics data" section. If a demo needs more than one truck or real routing, that's a signal to stand up `fleet-simulator-dev`, not to expand this stub.
